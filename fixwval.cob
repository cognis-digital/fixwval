...
*>
*> Mainframe-grade input sanitation: verifies that a numeric field at a fixed
*> column range is present and all-digits in every record of a flat file.
*> Catches malformed/garbage records before they hit a batch job.
*>
*> Usage:
...
MAIN-PARA.
    ...
    IF WS-PATH = SPACES OR WS-NSTART = 0 OR WS-NLEN = 0
        DISPLAY "usage: fixwval <datafile> <numStart> <numLen>"
            UPON SYSERR
    ...
    OPEN INPUT IN-FILE
    IF WS-FS NOT = "00"
        DISPLAY "fixwval: cannot open input file" UPON SYSERR
        MOVE 1 TO RETURN-CODE
        STOP RUN
    END-IF
...
EMIT-JSON.
    DISPLAY '{"tool":"fixwval","records":' WITH NO ADVANCING
